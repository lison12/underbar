(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    _.each(array, function(val) {
      if (_.indexOf(unique, iterator(val)) === -1) {
  ...
  _.map = function(collection, iterator) {
    ...
    _.each(collection, function(item, index, list) {
      results.push(iterator(item, index, list))
    })
    return results
  };
...
// Determine whether all of the elements match a truth test.
  ...
  // step 1
  // acc = true, item  = true -> true
  // step 2
  // acc = true, item = false -> false
  // step 3
  // acc = false, item = 1 -> true
  ...
  _.extend = function(obj) {
    var otherObjs = Array.from(arguments).slice(1)
    _.each(otherObjs, function(source) {
      for (var key in source) {
      //_.each(source, function(key) {
        obj[key] = source[key]
      }
    })
    return obj;
  ...
  _.memoize = function(func) {
    ...
    return function() {
      ...
      if (results[args] === undefined /*!results.hasOwnProperty(arguments) /*in results)/* have i not seen these arguements before*/) {
...
//417+621 -> 1038
  ...
  _.shuffle = function(array) {
    ...
    while (arrayCopy.length > 0) {
      ...
      randArray.push(arrayCopy[randIndex])
